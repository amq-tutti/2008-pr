...
function init() {
    ...
    document.querySelector('.finished.list.button.three').addEventListener('click', generateRanksByID);
    ...
    // Change video/mp3 sorter option before starting
    document.getElementById('mediaToggle').addEventListener('change', function (e) {
        isVideoMode = !e.target.checked;
    });
...
function start() {
    ...
    document.querySelector('.media-toggle').style.display = 'none';
...
function generateRanksByID() {
    const sortedCharacters = finalCharacters.sort((a, b) => a.id.localeCompare(b.id));
    const data = sortedCharacters.reduce((str, char) => {
        str += `${char.rank} ${char.name}<br>`;
        return str;
    }, '');
...
function generateTextList() {
    const sortedCharacters = finalCharacters.sort((a, b) => a.rank - b.rank);
    const data = sortedCharacters.reduce((str, char) => {
        str += `${char.rank}. ${char.name}<br>`;
        return str;
    }, '');
